...
class Player {
  ...
  strike(player, enemy, attackDmg) {
    ...
    let hpAmount = Math.ceil(Math.random() * attackDmg)
    // Subtract the enemy health with the damageAmount
    enemy.health -= hpAmount
    //  Update the game and DOM with updateGame()
    updateGame(p1, p2, game.isOver)
    //  Return a message of 'player name attacks enemy name for damageAmount'
    return `${player.name} attacks 
     ${enemy.name} for ${hpAmount} damage`
  ...
  heal(player) {
    ...
    let healAMt = Math.floor(Math.random() * 5)
    ...
    //  Update the game and DOM with updateGame()
    updateGame(p1, p2, game.isOver)
    //  Return a message of 'player name heals for 
    // hpAmount HP'
...
class Game {
  ...
  declareWinner(isOver, p1, p2) {
    ...
    if (game.isOver == true && p1.health <= 0) {
      message = `${p2.name}-- AND I AM IRON MAN`
    } else if (game.isOver == true && p2.health) {
      message = `${p1.name}-I CAN DO THIS ALLDAY`
    }
...
let Player1 = new Player("Captain", 100, 10)
let Player2 = new Player("Tony", 100, 10)
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key == "q" && game.isOver == false && p2.health > 0) {
    ...
    // After striking then play attac
